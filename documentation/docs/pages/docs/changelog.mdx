...
### Breaking changes
-------------------------------------------------
- breaking: **Exact Rust-indexing detail cursors** — `rindexer_internal` event cursor tables now key progress by `network` and opaque `detail_key`; downstream Rust callbacks and existing databases must migrate or replay each manifest detail instead of treating a network-wide cursor as exact coverage.
